...
class AvgDescriptor(DescriptorModel):
    ...
    def __init__(self, feat_arr, autofeats=None, sp_typemodel='matrix'):
        "The inputs are the needed to compute model_dim."
        self.features = self._format_features(feat_arr)
        if autofeats is not None:
            self.autofeats = self._format_features(autofeats)
            self.autoret = False
        else:
            self.autofeats = None
            self.autoret = True
